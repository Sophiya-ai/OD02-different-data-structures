...
class TreeNode:
    ...
    # метод `__repr__` — это специальный метод, который используется для получения канонического строкового представления объекта.
    # Основное назначение `__repr__` — предоставить подробное и однозначное представление объекта,
    # которое может быть использовано для его воссоздания.

    # Цель `__repr__` — предоставить такое строковое представление объекта, которое максимально точно описывает его.
    # Идеально, если строка, возвращаемая `__repr__`, может быть использована для создания эквивалентного объекта,
    # если ее передать в функцию `eval()`. Это означает, что `eval(repr(obj)) == obj` должно быть верно, если это возможно

    # `__repr__` предназначен для использования в основном разработчиками, чтобы предоставить детальную информацию об объекте.
    # Это полезно для отладки и логирования.
    # **Отладка**: Когда вы печатаете список объектов или когда объект отображается в интерактивной оболочке,
    # Python вызывает `__repr__` для получения строкового представления объектов

    # Если в классе определен только `__repr__`, но не `__str__`, то при вызове `str()` или `print()` будет использоваться `__repr__`.
    def __repr__(self):
